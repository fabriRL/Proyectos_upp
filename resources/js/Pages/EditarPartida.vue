...
const form = reactive({ presupuesto_aprobado: Number(props.partida.presupuesto_aprobado) || 0 })
const errors = ref({})
...
async function guardar() {
  ...
  errors.value = {}
  try {
    await axios.put(`/api/partidas/${props.partida.id_partida}`, form)
    showToast('Partida presupuestaria actualizada correctamente.', 'success')
    emit('updated')
    emit('close')
  } catch (e) {
    if (e.response?.status === 422) errors.value = e.response.data.errors
      ...
      console.error(e)
      showToast('No se pudo actualizar la partida.', 'error')
    }
  } finally {
...
<template>
  <Teleport to="body">
    ...
    <div :style="sOverlay" @click.self="cerrar">
      <div :style="sBox">
        <h2 style="margin:0 0 4px;font-size:1.1rem;">Editar partida presupuestaria</h2>
        <p style="margin:0 0 16px;font-size:0.72rem;color:#8ea9bf;">
          Al cambiar el presupuesto aprobado se recalcula el saldo por ejecutar de todos los objetos de gasto de esta partida.
        </p>
